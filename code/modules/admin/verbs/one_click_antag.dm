...
/datum/admins/proc/makeFirstTeam()
	var/list/mob/dead/observer/candidates = pollGhostCandidates("Do you wish to be considered for a squad of Pentex First Team Operators?", ROLE_FIRST_TEAM, null)
	var/list/mob/dead/observer/chosen = list()
	var/mob/dead/observer/theghost = null
	...
	if(candidates.len)
		...
		for(var/i = 0, i<numagents,i++)
			...
			for(var/mob/j in candidates)
				...
				agentcount++
				break
		//Making sure we have atleast 3 First Team agents, because less than that is kinda bad
		if(agentcount < 1)
			return FALSE
